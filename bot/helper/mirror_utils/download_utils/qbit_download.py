...
from bot import parent_id, DOWNLOAD_DIR, IS_TEAM_DRIVE, INDEX_URL, \
    USE_SERVICE_ACCOUNTS, ENABLE_DRIVE_SEARCH,  download_dict, download_dict_lock, DOWNLOAD_STATUS_UPDATE_INTERVAL, MAX_TORRENT_SIZE
...
class QbitWrap:
            ...
            try:
                ...
                if tor_info.state != "metaDL":
                    if self.sizeavail:
                        ...
                        if tor_info.total_size > free:
                            self.__onDownloadError(f"<b>Size of {tor_info.name} Exceeds The Free Space on Disk</b>. <i>Thus Download Stopped!.</i>")
                            client.torrents_delete(torrent_hashes=tor_info.hash,delete_files=True)
    ...
    def get_confirm_callback(self, e,lis):
        ...
        if o_sender != str(e.sender_id):
            e.answer("Dont Touch it.......")
            return
        if str(lis[2]) != msgid:
            return
        e.answer("Starting the download with the selected files.")
        lis[0] = True
